...
class CaseClassifier:
    ...
    @classmethod
    def extract_amount(cls, text: str) -> tuple[float, str]:
        """从文本中提取涉案金额"""
        import re
        # 按优先级匹配：亿 > 万 > 元，保留最大金额
        patterns = [
            (r'(\d+(?:,\d{3})*(?:\.\d+)?)\s*亿', 100000000),
            (r'(\d+(?:,\d{3})*(?:\.\d+)?)\s*万\s*元', 10000),
            (r'人民币\s*(\d+(?:,\d{3})*(?:\.\d+)?)\s*万', 10000),
            (r'(\d+(?:,\d{3})*(?:\.\d+)?)\s*万元', 10000),
            (r'(\d+(?:,\d{3})*(?:\.\d+)?)\s*千元', 1000),
            (r'(\d+(?:,\d{3})*(?:\.\d+)?)\s*元', 1),
        ]
        max_val = 0
    ...
    @classmethod
    def extract_party_count(cls, text: str) -> tuple[int, str]:
        """从文本中提取当事人数"""
        import re
        patterns = [
            r'[^甲乙丙丁](?:甲乙丙丁[^戊己庚]?){0,5}(?:共|共计|合计)\s*(\d+)\s*(?:人|方|名)',
            r'(\d+)\s*(?:人|方|名)\s*(?:参与|当事人|原告|被告)',
            r'原告\s*(?:约)?\s*(\d+)\s*(?:人|家|个)',
            r'被告\s*(?:约)?\s*(\d+)\s*(?:人|家|个)',
        ]
        for pattern in patterns:
            ...
            if m:
                try:
                    ...
                    pass
        # 简单估算：文本中"甲"、"乙"等出现次数
        parties = set(re.findall(r'[甲乙丙丁戊己庚]', text))
        count = max(len(parties), 2)
        return count if count > 0 else 0, "约" if count > 2 else ""

    @classmethod
    def extract_domain(cls, text: str) -> str:
        """从文本中识别法律领域"""
        domain_map = {
            "合同纠纷": ["合同", "违约", "违约金", "合同效力", "合同解除"],
            "婚姻家庭": ["离婚", "抚养", "继承", "遗产", "婚姻", "夫妻"],
            "交通事故": ["交通事故", "肇事", "保险", "赔偿", "伤残"],
            "劳动争议": ["劳动", "工伤", "劳动合同", "劳务", "工资", "社保"],
            "房产纠纷": ["房屋", "房产", "买卖", "租赁", "物业", "车位"],
            "债务纠纷": ["借款", "欠款", "债务", "借贷", "利息", "本金"],
            "侵权纠纷": ["侵权", "赔偿", "损失", "人身", "财产"],
            "刑事案件": ["盗窃", "诈骗", "抢劫", "伤害", "刑事", "犯罪"],
        }
        text_lower = text
        best_domain = "综合"
        best_score = 0
...
def create_app(fayan: FaYanLegal):
    ...
    def classify(req: ClassifyRequest):
        ...
        amount, amount_reason = CaseClassifier.extract_amount(req.case_text)
        party_count, party_reason = CaseClassifier.extract_party_count(req.case_text)
        domain = CaseClassifier.extract_domain(req.case_text)
