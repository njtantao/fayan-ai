...
BASE_DIR = Path("/Users/tt/Downloads/法院全量数据/2.中国法院2015年度案例19册")
OUT_JSON = Path("/Users/tt/Desktop/hermes/项目开发/法眼ai代码/extracted_cases/all_cases.json")

PDF_FILES = [
    ("中国法院2015年度案例（1）婚姻家庭与继承纠纷.pdf", "婚姻家庭与继承纠纷"),
    ("中国法院2015年度案例（2）物权纠纷（238页）.pdf", "物权纠纷"),
    ("中国法院2015年度案例（3）土地纠纷.pdf", "土地纠纷"),
    ("中国法院2015年度案例（4）房屋买卖合同纠纷.pdf", "房屋买卖合同纠纷"),
    ("中国法院2015年度案例（5）合同纠纷.pdf", "合同纠纷"),
    ("中国法院2015年度案例（6）买卖合同纠纷.pdf", "买卖合同纠纷"),
    ("中国法院2015年度案例（7）借款担保纠纷.pdf", "借款担保纠纷"),
    ("中国法院2015年度案例（8）民间借贷纠纷.pdf", "民间借贷纠纷"),
    ("中国法院2015年度案例（9）侵权赔偿纠纷（237页）.pdf", "侵权赔偿纠纷"),
    ("中国法院2015年度案例（10）道路交通纠纷（241页）.pdf", "道路交通纠纷"),
    ("中国法院2015年度案例（11）雇员受害赔偿纠纷（211页）.pdf", "雇员受害赔偿纠纷"),
    ("中国法院2015年度案例（12）人格权纠纷（213页）.pdf", "人格权纠纷"),
    ("中国法院2015年度案例（13）劳动纠纷（217）.pdf", "劳动纠纷"),
    ("中国法院2015年度案例（14）公司纠纷（241页）.pdf", "公司纠纷"),
    ("中国法院2015年度案例（15）保险纠纷（291页）.pdf", "保险纠纷"),
    ("中国法院2015年度案例（16）金融纠纷（285页）.pdf", "金融纠纷"),
    ("中国法院2015年度案例（17）行政纠纷（329页）.pdf", "行政纠纷"),
    ("中国法院2015年度案例（18）刑法总则案例（301页）.pdf", "刑法总则案例"),
    ("中国法院2015年度案例（19）刑法分则案例（282页）.pdf", "刑法分则案例"),
]
...
def parse_one_case(case_text, category, idx):
    ...
    if case_num_m:
        case_number = f"{case_num_m.group(1)}-15-2-{case_num_m.group(2).zfill(3)}-{category[:4]}"
    else:
        case_number = f"2015-15-2-{idx:04d}-{category[:4]}"
    cause = ''
    for pat in [r'案由\s*[:：]\s*([^\n【】]{2,40})', r'案由[:：]\s*([^\n]{2,30})']:
...
def main():
    ...
    for filename, category in PDF_FILES:
        ...
        for idx, (seg_type, start_pos) in enumerate(info_starts):
            ...
            case = parse_one_case(case_text, category, idx)
            is_dup = any(
        ...
        total_new += added
        print(f"  [{category}] 新增 {added}, 跳过 {skipped}")
